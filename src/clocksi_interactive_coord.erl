...
wait_for_start_transaction({call, Sender}, {start_tx, ClientClock, Properties}, _State) ->
    ...
    case {Locks, SharedLocks, ExclusiveLocks} of
            ...
            {L, SL, EL} ->
                State = start_tx_internal_with_locks(ClientClock, Properties, BaseState, L, SL, EL),
                case State of
                    {ok, State} ->
                        TxnId = (State#state.transaction)#transaction.txn_id,
                        {next_state, execute_op, State, {reply, Sender, {ok, TxnId}}};
                    Error ->
                        Sender ! Error
                        %{next_state, aborted, #state{state = aborted}, {reply, Sender, Error}}
                end
    end.
...
start_tx_internal_with_locks(
    ...
    case create_transaction_record_with_locks(ClientClock, Properties, Locks, Shared_Locks, Exclusive_Locks) of
        {ok, Transaction, TransactionId} ->
            % a new transaction was started, increment metrics
            ?STATS(antidote_open_transactions),
            {ok, State#state{transaction = Transaction, num_to_read = 0, properties = Properties, transactionid = TransactionId}};
        {locks_not_available, Missing_Locks} ->    % TODO is this the right way to abort the transaction if it was not possible to acquire the locks
            %From ! {error, Missing_Locks},
            {stop, "Missing Locks: " ++ lists:flatten(io_lib:format("~p", [Missing_Locks]))};
        {locks_in_use, Tx_Using_The_Locks} ->    % TODO is this the right way to abort the transaction if it was not possible to acquire the locks
...
%% #Locks
prepare_transaction_with_locks(ClientClock, Properties) ->
    ...
    Transaction = #transaction{snapshot_time_local = LocalClock,
        ...
        properties = Properties},
    {Transaction, LocalClock, SnapshotTime}.

get_locks_from_properties(LockPropertyName, Properties) ->
    _ = case lists:keyfind(LockPropertyName, 1, Properties) of
        {_LockPropertyName, FindLocks}
            when is_list(FindLocks) -> FindLocks;
        _Else -> []
    end.

create_transaction_record_with_locks(ClientClock, Properties, Locks, [], []) ->
    {TransactionId, LocalClock, SnapshotTime} = prepare_transaction_with_locks(ClientClock, Properties),
...
create_transaction_record_with_locks(ClientClock, Properties, [], SharedLocks, ExclusiveLocks) ->
    {TransactionId, LocalClock, SnapshotTime} = prepare_transaction_with_locks(ClientClock, Properties),
...
create_transaction_record_with_locks(ClientClock, Properties, Locks, SharedLocks, ExclusiveLocks) ->
    {TransactionId, LocalClock, SnapshotTime} = prepare_transaction_with_locks(ClientClock, Properties),
...
start_link_with_locks_available_test() ->
    ...
    {ok, Pid} = clocksi_interactive_coord:start_link(),
    {ok, _Tx} = gen_statem:call(Pid, {start_tx, Clientclock, Properties}),
...
start_link_with_locks_not_available_test() ->
    ...
    {ok, Pid} = clocksi_interactive_coord:start_link(),
    {_, _Tx} = gen_statem:call(Pid, {start_tx, Clientclock, Properties}),
