...
requested(Locks, DcId, Timestamp, Lock_Requests) ->
    case orddict:find(DcId, Lock_Requests) of
        ...
        error ->
            ...
            New_Lock_Request_Orddict = lists:foldl(fun(Elem, AccIn) ->
                orddict:store(Elem, Timestamp, AccIn) end, New_Orddict, Locks),
...
clear_old_lock_requests(Lock_Requests, Timeout) ->
    ...
    orddict:fold(
        fun(DCID, Lock_Timestamp_List, AccIn) ->
            Filtered_Orddict = orddict:filter(fun(_Lock_2, Timestamp_2) ->
                timer:now_diff(Current_Time, Timestamp_2) < Timeout end, Lock_Timestamp_List),
